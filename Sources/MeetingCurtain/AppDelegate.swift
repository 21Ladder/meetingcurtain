...
final class AppDelegate: NSObject, NSApplicationDelegate {
    static let openSettingsNotification = Notification.Name("com.manu.meetingcurtain.openSettings")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        Task {
            ...
            if !UserDefaults.standard.bool(forKey: Self.firstRunKey) {
                ...
                // Right after login the calendar database may still be starting up; only bother the
                // user if the problem is still there a little later.
                try? await Task.sleep(for: .seconds(20))
                await monitor.runSelfCheck()
                if monitor.health.overall == .critical { settings.show() }
            }
        }
    }
